...
def sshcmd(ip,port,username,pwd):
    ...
    tmplist["ip"]=ip
    tmplist["cpuuse"]=None
    tmplist["memoryall"] = None
    tmplist["memoryuse"] = None
    ...
    try:
        ...
        stdincpu,stdoutcpu,stderrcpu=ssh.exec_command("top -b -n 1|head -n 4|grep 'Cpu(s)'|awk '{print $2}'|cut -d 'u' -f 1")
        stdinmemoryall, stdoutmemoryall, stderrmemoryall = ssh.exec_command("free -m | awk '{print $2}' | awk 'NR==2{print}'")
        stdinmemory, stdoutmemory, stderrmemory = ssh.exec_command("echo `free -m | sed -n '2p' | awk '{print  ($2-$7)/$2*100}'|awk -F'.' '{print $1}'`%")
        ...
        stdinhomeall, stdouthomeall, stderrhomeall = ssh.exec_command("df -hP /home | awk '{print $2}' | awk 'NR==2{print}'")
